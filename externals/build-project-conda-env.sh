...
# Ensure the required channels are added to conda
if [ -f $CONDA_REQUIREMENTS_DIR/channels ]
then
    for i in $(cat $CONDA_REQUIREMENTS_DIR/channels)
    do
        $CONDA_PATH config --add channels "$i"
    done
fi
...
then
	...
	pip install $PIP_INSTALL_OPTIONS -r $PIP_REQUIREMENTS_DIR/$ENV_TYPE.txt
fi

printf "\033[32m --- eva created\033[0m"
